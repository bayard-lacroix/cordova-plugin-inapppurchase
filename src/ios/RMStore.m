...
- (BOOL)paymentQueue:(SKPaymentQueue *)queue shouldAddStorePayment:(nonnull SKPayment *)payment forProduct:(nonnull SKProduct *)product
{
    // This delegate method adds support for Promoting In-App Purchases. See:
    // https://developer.apple.com/library/archive/documentation/NetworkingInternet/Conceptual/StoreKitGuide/PromotingIn-AppPurchases/PromotingIn-AppPurchases.html
    RMStoreLog(@"got paymentQueue:shouldAddStorePayment:forProduct: productId=%@", product.productIdentifier);
    return YES;
}
